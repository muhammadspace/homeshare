...
owners_interests_tfidf = tfidf_vectorizer.fit_transform(owners_df['All Traits'])

# TF-IDF encoding for seeker interests
seekers_interests_tfidf = tfidf_vectorizer.transform(seekers_df['All Traits'])
...
def recommend_seekers_traits(owner_id):
    ...
    if not owner.index.empty:
        ...
        if len(sorted_indices) > 0:
            ...
            for idx in sorted_indices:
                ...
                common_interests = sum(1 for interest in owners_df.iloc[owner_index]['All Traits'].split(', ') if interest in seekers_df.iloc[idx]['All Traits'].split(', '))
